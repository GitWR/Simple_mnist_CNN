...
Y1 = tf.nn.relu(tf.nn.bias_add(cov1, B1))
...
Y2 = tf.nn.relu(tf.nn.bias_add(cov2, B2))
pool_2 = tf.nn.max_pool(Y2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')  # 4 * 4
# Y2_drop = tf.nn.dropout(pool_2, p_drop)

stride_3 = 2
cov3 = tf.nn.conv2d(pool_2, W3, strides=[1, stride_3, stride_3, 1], padding='SAME')
Y3 = tf.nn.relu(tf.nn.bias_add(cov3, B3))
...
with tf.Session() as sess:
    ...
    for i in range(epoch):
        ...
        for j in range(n_batch):
            ...
            P_value, loss_value = sess.run([parameters, loss], feed_dict={X: X_batch, Y_practical: Y_batch, p_drop: 0.5})
        ...
        print("step %d: loss value is %f" % (i, sum_each_batch/n_batch))
        print("\n step %d: learning rate is :%f" % (i, learning_rate))
    ...
    for k in range(n_batch_test):
        ...
        Y_output = sess.run(Y5, feed_dict={X: X_batch_test, Y_practical: Y_batch_test, p_drop: 1})
    ...
    print("The final classification accuracy of the %d epochs is %f" % (epoch, correct_num / m_data.test.num_examples))
